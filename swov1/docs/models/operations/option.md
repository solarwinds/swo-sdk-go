...
```go
swov1.WithServerURL("https://api.example.com")
```
...
```go
swov1.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
swov1.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
swov1.WithTimeout(30 * time.Second)
```
